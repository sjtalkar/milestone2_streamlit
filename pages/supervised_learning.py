...
from lib.prediction_visualization import (
    get_geo_prediction_df,
    draw_predictions_for_model,
    get_evaluation_error_metrics,
)
...
def app():
    ...
    county_selected = st.selectbox(
        "View year 2021 predictions for county:", ["All"] + counties_list
    )
    model_selected = st.selectbox("View year 2021 predictions from model:", model_list)

    new_y_pred_df = y_pred_df.copy()
    if county_selected != "All":
        new_y_pred_df[model_selected] = np.where(
            new_y_pred_df["COUNTY"] != county_selected, 0, new_y_pred_df[model_selected]
        )
    ...
    if (
        st.button("View Predictions")
        or st.session_state.model_loaded
        or st.session_state.county_loaded
    ):
        ...
        st.caption(
            f"The prediction values are not absolute. The error metrics for each of the models is tabulated below."
        )
        st.markdown("""---""")
        ...
        if county_selected == "All":
            ...
            st.dataframe(
                new_df.loc[new_y_pred_df["COUNTY"] == county_selected][
                    ["COUNTY", "TOWNSHIP_RANGE", "YEAR", model_selected]
                ]
            )
    ...
    st.caption("Click on column header for sort. Columns can also be resized.")

    error_eval_df = get_evaluation_error_metrics().sort_values(
        by=["Mean Absolute Error"]
    )
    # The mean of the target depth = 167
    error_eval_df["% MAE error relative to mean"] = (
        error_eval_df["Mean Absolute Error"] / 167 * 100
    ).astype("int")
...
non-constant variance and non-normal distributions.
    ...
    with col2:
        st.markdown(
            """Adjacent figure shows prediction values in each quantile
...
Some other correlations that were noticed to be higher
...
### R-squared and MAE
For the initial comparison of algorithms, before hypertuning and for generating a list of models for
PyCaret to compare, R-squared gave us a quick indication of the relative performance.
R-squared provides the proportion of the variance for the target that’s explained by selected 
features in the model. An R-squared value of 0.9, for instance, would indicate that 90% of the variance 
of the dependent variable being studied is explained by the variance of the independent variable.
It is independent of the scale of the features and ranges from 0 to 1. A negative value implies worse
than the mean model. 

When evaluating an algorithm, it is prudent to look into multiple regression scores and not just R-squared 
since the acceptable threshold of the error will depend on the distribution of the target value itself.
To keep the groundwater depth prediction difference from the mean in check, we additionally look into MAE,
MSE and RMSE and communicate the percentage mean absolute error relative to the mean of the true target value
...
well the data fit the regression model."""
        ...
        st.markdown(
            """After evaluation on both train and test set results and a combination of evaluation metrics, R-squared and MAE,
